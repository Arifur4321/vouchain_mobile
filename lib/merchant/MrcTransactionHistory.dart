...
class MrcTransactionHistory extends StatefulWidget {
  final MerchantDTO mrc;

  MrcTransactionHistory({Key key, @required this.mrc}) : super(key: key);

  @override
  _MrcTransactionHistoryState createState() => _MrcTransactionHistoryState();
}

class _MrcTransactionHistoryState extends State<MrcTransactionHistory> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
        drawer: MrcDrawerMenu(selected: 3, mrc: widget.mrc),
        appBar: AppBar(
  ...
  Future<DTOList<dynamic>> _updateList(
    ...
    return MrcServices.mrcTransactionList(_start, _end, widget.mrc, context);
  }
  ...
  // Mrc = Riscatto buoni o mittente
  String _getTrcDescription(TransactionDTO trc, BuildContext context) {
    switch (trc.trcType) {
      case 'SPS':
        {
          return VouchainLocalizations.of(context).reciveVoucherFrom +
              trc.usrIdDaString;
        }
      case 'RED':
        {
          return VouchainLocalizations.of(context).redeemVoucher;
        }
      default:
        {
            ...
            children: [
              Container(
                ...
                child: Column(
                  ...
                  children: [
                    if (trc.trcType == "SPS") _trcDetailsSender(trc),
                    Material(
                      elevation: 1,
                      child: Container(
                        color: colors.lightBlue,
  ...
  _trcDetailsSender(TransactionDTO trc) {
    Future<EmployeeDTO> futureEmployee =
        EmpServices.getEmpProfile(trc.usrIdDa, context: context);
    ...
    return Material(
      ...
      child: Container(
        ...
        child: ExpansionTile(
          title: Center(
            child: Text(
              VouchainLocalizations.of(context).recipientSender,
              style: TextStyle(
                fontWeight: FontWeight.w600,
                fontSize: 52.5.sp,
                color: colors.blue,
  ...
  void _exportHistory() async {
    ...
    if (await Permission.storage.request().isGranted) {
      ...
      final encodedStr = await UserServices.exportHistory(
          "merchant", selectedRange, context, widget.mrc);
      if (encodedStr == "empty") {
        return showDialog(
          barrierDismissible: true,
          context: context,
